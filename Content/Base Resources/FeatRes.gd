...
# Variables
# If requirements are empty, and the frequency is 0, then the feat is permanently active.
...
func _init(name : String = "", desc : String = "", _traits : Array = [], 
			...
			_modifiers : Dictionary = {}, _extra: Dictionary = {}) -> void:
	f_name = name
	description = desc
	traits = _traits
...
# Get the name of the feat
# Args: None
# Returns: String - Name of the feat
...
func get_modifier(modifier: String) -> int:
	if modifiers.has(modifier):
		...
		ErrorUtility.log_warning("Modifier '%s' not found in feat '%s'" % [modifier, f_name])
		return 0
...
func get_cost(cost: String) -> int:
	if costs.has(cost):
		...
		ErrorUtility.log_warning("Cost " + cost + " not found in feat " + f_name)
		return 0
...
func get_costs() -> Array:
	...
	if actual_costs.is_empty():
		ErrorUtility.log_warning("No costs found for feat '" + f_name + "'")
		return []
