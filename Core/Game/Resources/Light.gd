...
func change_light_intensity(new_intensity: float) -> void:
	light_intensity = new_intensity
	_change_shader_parameter("intensity", new_intensity)

func change_light_attenuation_strength(new_attenuation_strength: float) -> void:
...
func change_light_brightness(new_brightness: float) -> void:
	light_brightness = new_brightness
	_change_shader_parameter("brightness", new_brightness)

func change_light_visibility(new_visibility: bool) -> void:
...
func update_state(updated_values: Dictionary) -> void:
	if Net.is_host():
		return

	if updated_values.has("position"):
		change_light_position(updated_values["position"])
	if updated_values.has("radius"):
		change_light_radius(updated_values["radius"])
	if updated_values.has("color"):
		change_light_color(updated_values["color"])
	if updated_values.has("intensity"):
		change_light_intensity(updated_values["intensity"])
	if updated_values.has("attenuation_strength"):
		change_light_attenuation_strength(updated_values["attenuation_strength"])
	if updated_values.has("brightness"):
		change_light_brightness(updated_values["brightness"])
